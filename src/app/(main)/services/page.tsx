import type { Metadata } from "next"
import { SITE_NAP, SITE_SLUGS } from "@/config/site-config"

export const metadata: Metadata = {
  title: `${SITE_NAP.name} | Services`,
  description: `Services for ${SITE_NAP.name}`,
  alternates: {
    canonical: SITE_SLUGS.allServices,
  },
}

const ServicesPage: React.FC = () => {
  return (
    <main className="mx-auto min-h-screen w-full max-w-5xl px-6 pt-28 pb-16">
      <h1 className="font-bold text-4xl text-foreground">Services</h1>
      <p className="mt-4 max-w-2xl text-foreground/80 text-lg">
        Replace this with the client&apos;s service groups, supporting details, and clear calls to
        action for each offer.
      </p>
    </main>
  )
}

export default ServicesPage

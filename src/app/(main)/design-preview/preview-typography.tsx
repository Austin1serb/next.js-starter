...
const scale: { className: string; label: string; range: string; sample: string }[] = [
  ...
  { className: "text-caption tracking-wider uppercase", label: "text-caption", range: "11 → 13", sample: "The quick brown fox" },
]
      ...
      <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          ...
          <p className="text-body text-foreground-muted mt-4 max-w-prose">
            Every size scales fluidly between mobile and desktop using <code className="text-body-sm bg-surface rounded-sm px-1.5 py-0.5">clamp()</code>. Display
            sizes use <code className="text-body-sm bg-surface rounded-sm px-1.5 py-0.5">font-display</code>; everything else uses{" "}
          ...
          <ul className="mt-10 space-y-8">
            {scale.map((row) => (
              <li key={row.label} className="border-border flex flex-col gap-2 border-b pb-6 last:border-b-0">
                ...
                <p className={`${row.className} text-foreground`}>{row.sample}</p>
              </li>
            ))}
          </ul>
        </div>

        <aside className="bg-surface border-border rounded-xl border p-8 shadow-sm">
